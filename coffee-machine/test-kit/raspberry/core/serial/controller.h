...
public:
    ...
    void dataReceived(uint16_t length, uint16_t type, uint8_t* data)
    {
        if (!receivedHandler_.empty()) receivedHandler_(length, type, data);
    }

    void setReceivedHandler(boost::function<void(uint16_t, uint16_t, uint8_t*)> handler){receivedHandler_ = handler;}

private:
    boost::function<void(uint16_t, uint16_t, uint8_t*)> receivedHandler_;
