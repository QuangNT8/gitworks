...
void motor::Controller::loop()
{
    ...
    if(motorEnabled_)
    {
        if ((abs(ia_) > CURRENT_THRES) || (abs(ib_) > CURRENT_THRES))
        {
            stopMotor();
            //motorError_();
    ...
    switch (state_)
    {
        ...
        case WAIT_FOR_STABILITY:
        {
            ...
            if (count++>100)
            {
                ...
                if(isZero)
                {
                    uart::CONTROLLER.printfMessage("isZero = 1");
                    state_ = FORWARD_TO_ZERO;
                    generator_.start(1, 200, -4096*1024);
                }
                else
                {
                    uart::CONTROLLER.printfMessage("isZero = 0");
        ...
        case MOVE_TO_HOME:
        {
            uart::CONTROLLER.printfMessage("MOVE_TO_HOME");
        ...
        case HOMING:
        {
            //uart::CONTROLLER.printfMessage("HOMING");
        ...
        case MOVING:
        {
            if (generator_.state() == motor::Generator::NONE)
            {
                uart::CONTROLLER.printfMessage("!MOVING");
