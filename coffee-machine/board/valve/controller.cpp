...
void valve::Controller::loop()
{
    if (needUpdateTime_)
    {
        ...
        for(int i=MIN_VALVE_INDEX; i<MAX_VALVE_INDEX; i++)
        {
            if (times_[i] > 0) times_[i]--;
...
void valve::Controller::moveLeft_(uint16_t index)
{
    leftBits_  |= (1U << index);
    midleBits_ &= (~(1U<< index));
}

void valve::Controller::moveRight_(uint16_t index)
{
    leftBits_  &= (~(1U << index));
    midleBits_ &= (~(1U << index));
}

void valve::Controller::moveMiddle_(uint16_t index)
{
    midleBits_ |= (1U << index);
    leftBits_  &= (~(1U << index));
}

void valve::Controller::open(uint16_t index, uint16_t time)
{
    ...
    if (time > 0)
    {
        times_[index] = time;
        state_[index] = OPEN;
        moveRight_(index); //FIXME: RIGHT = open?
        valveOpened_(index);
    }
}
...
{
    for (int i =0;i< MAX_VALVE_INDEX; i++)
    {
        times_[i] = 0;
        state_[i] = CLOSE;
    }
