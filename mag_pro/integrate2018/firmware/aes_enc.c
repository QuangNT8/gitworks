...
int32 B0(int32 x) { return (x&255); }
int32 B1(int32 x) { return ((x>>8)&255); }
int32 B2(int32 x) { return ((x>>16)&255); }
int32 B3(int32 x) { return ((x>>24)&255); }

int32 F1(int32 x0,int32 x1,int32 x2,int32 x3)
{
  return B1(T1[x0&255]) | (B1(T1[(x1>>8)&255])<<8)
      | (B1(T1[(x2>>16)&255])<<16) | (B1(T1[x3>>24])<<24);
}

int32 packBytes(unsigned int8 *octets)
{
  int32 i, j;
  int32 len=octets.length;
  int32 b=new Array(len/4);
...
int32 unpackBytes(int32 packed)
{
  uint32 j;
  uint32 i=0, l = packed.length;
  uint32 r = new Array(l*4);
...
function keyExpansion(key)
{
  int32 kc, i, j, r, t;
  int32 rounds;
  int32 keySched=new Array(maxrk+1);
  int32 keylen=key.length;
  int32 k=new Array(maxkc);
  int32 tk=new Array(maxkc);
  int32 rconpointer=0;
...
int32 AESencrypt(int32 block,int32 ctx)
{
  uint8 r;
  uint8 t0,t1,t2,t3;

  uint8 b = packBytes(block);
  uint8 rounds = ctx.rounds;
  uint8 b0 = b[0];
  uint8 b1 = b[1];
  uint8 b2 = b[2];
  uint8 b3 = b[3];
