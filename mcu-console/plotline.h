...
class OscLine: public DataLine
{
public:
    OscLine(QXYSeries* series): DataLine(series)
    {
        plotData_.resize(1000);

        for (int i=0;i<1000;i++) data_[i] = 0;
    }
    void addValue(uint32_t x, int32_t y)
    {
        if (x<1000) data_[x] = y;
    ...
    void flush(uint32_t firstIndex)
    {
        //QVector<QPointF> plotData_(1000);
        for (int i=0;i<1000;i++)
        {
            plotData_[i].setX(i);
            plotData_[i].setY(data_[i]);
        }
        series_->replace(plotData_);
    }
private:
